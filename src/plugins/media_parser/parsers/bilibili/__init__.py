...
class BilibiliParser(BaseParser):
    ...
    async def parse_video(
        ...
        *,
        ...
        # UP
        pendant_url = await self._get_pendant_url(video_info.owner.mid)
    ...
    async def _get_pendant_url(self, mid: int) -> str | None:
        """Fetch the avatar pendant from Bilibili's public user-card API."""
        try:
            ...
            if not isinstance(payload, dict):
                return None
            pendant = payload.get("data", {}).get("card", {}).get("pendant")
            if not isinstance(pendant, dict):
                return None
            image = pendant.get("image")
            return image if isinstance(image, str) else None
        except (HTTPError, ValueError, AttributeError) as error:
            logger.debug(f"获取 Bilibili 头像挂件失败: {error}")
            return None
    ...
    async def credential(self) -> Credential | None:
        ...
        if await self._credential.check_refresh():
            ...
            if self._credential.has_ac_time_value() and self._credential.has_bili_jct():
                ...
                logger.info("哔哩哔哩凭证刷新成功，仅在当前进程内有效")
            else:
