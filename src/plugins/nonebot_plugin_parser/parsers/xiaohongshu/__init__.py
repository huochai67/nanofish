...
from ..base import BaseParser, ParseException, Platform, PlatformEnum, handle, pconfig
from .common import InteractInfo
...
class XiaoHongShuParser(BaseParser):
    ...
    async def _parse_profile(self, _searched: re.Match[str]):
        raise TipException("小红书用户主页暂不支持解析，请分享笔记或视频链接")
    ...
    async def parse_explore(self, url: str, xhs_id: str):
        ...
        stats = self._interact_stats(note_detail.interactInfo)
        result = self.result(
            author=author,
    ...
    async def parse_discovery(self, url: str):
        ...
        stats = self._interact_stats(note_data.interactInfo)
        result = self.result(
            author=author,
    ...
    @staticmethod
    def _interact_stats(interact_info: InteractInfo | None) -> dict[str, int]:
        if interact_info is None:
            return {}

        def count(value: int | str | None) -> int:
            try:
        ...
        return {
            "like": count(interact_info.likedCount),
            "collect": count(interact_info.collectedCount),
            "comment": count(interact_info.commentCount),
            "share": count(interact_info.shareCount),
        }
